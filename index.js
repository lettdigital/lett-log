...
const myFormat = printf(log => {
    return moment().utc().format('YYYY-MM-DD HH:mm:ss,SSSZ') + `[${log.level.toUpperCase()}][${log.label}]:${log.message}`;
});
...
class Log {
    ...
    constructor(defaultMeta = {}, { appName = '', host = '', protocol = '', port = 0, facility = '', path = '' } = {}) {
        this.logger = winston.createLogger({
            ...
            level: 'debug',
            format: combine(
                label({ label: appName || SYSLOG_APP_NAME }),
                myFormat,
            ),
            transports: [
    ...
    /**
     * @function show A generic log function
    ...
    /**
     * @func info Log level info - priority 3
     * @param {String} msg A message which will be not indexed by elastic
     * @param {Object} metadata A JSON which will be indexed by elastic
     */
    info(msg, metadata) {
    ...
    /**
     * @func warn Log level warn - priority 1
     * @param {String} msg A message which will be not indexed by elastic
     * @param {Object} metadata A JSON which will be indexed by elastic
     */
    warn(msg, metadata) {
    ...
    /**
     * @func alert Log level alert - priority 2
     * @param {String} msg A message which will be not indexed by elastic
     * @param {Object} metadata A JSON which will be indexed by elastic
     */
    alert(msg, metadata) {
    ...
    /**
     * @func debug Log level debug - priority 4
     * @param {String} msg A message which will be not indexed by elastic
     * @param {Object} metadata A JSON which will be indexed by elastic
     */
    debug(msg, metadata) {
    ...
    /**
     * @func error Log level error - priority 0
     * @param {String} msg A message which will be not indexed by elastic
     * @param {Object} metadata A JSON which will be indexed by elastic
    ...
    /**
     * @func toError Parse every property from error to string
     * @param {Error} err 
